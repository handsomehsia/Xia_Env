...
class Xia1(gymnasium.Env):
    def __init__(self) -> None:
        ...
        self.observation_space = gymnasium.spaces.Box(
            low=0.0, high=1.0, shape=(config["observation_size"],))
        self.config = config
    ...
    def default_config(cls):
        ...
        uav_action_lst = list(itertools.product(
            [-10, 0, 10], [-10, 0, 10], [-5, 0, 5]))
    ...
    def BMapChange1(self, uav):
        ...
        search_banjing = uav.z * \
            self.config['search_banjing_max'] / self.config['h_3d']
    ...
    def BMapChange(self, uav):
        ...
        # 標準差可以改一下，也跟距離有關
        noise = np.clip(np.random.normal(80/uav.z, 2*uav.z*0.01, size=A.shape)/10, -1, 1)
...
noise = np.clip(np.random.normal(80/20, 2*20*0.01, size=(3,3))/10, -1, 1)