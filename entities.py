...
class People:
    ...
    def reset(self) -> None:
        ...
        self.x = int(self.rng.uniform(self.search_banjing_max,
                     self.width - self.search_banjing_max))    # 考虑探索半径
        self.y = int(self.rng.uniform(self.search_banjing_max,
                     self.height - self.search_banjing_max))
...
class UAV:
    ...
    def reset(self) -> None:
        ...
        self.x = int(self.rng.uniform(self.search_banjing_max,
                     self.width - self.search_banjing_max - 1))    # 考虑探索半径
        self.y = int(self.rng.uniform(self.search_banjing_max,
                     self.height - self.search_banjing_max - 1))
        self.z = int(self.rng.uniform(self.h_min, self.h_3d)
                     )        # uav 限制高度大于self.h_min
    ...
    def move(self, action: Tuple[float, float, float] = None) -> None:
        ...
        position = np.clip(position, self.search_banjing_max, max(
            self.width, self.height) - self.search_banjing_max - 1)
